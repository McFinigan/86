...
class Acceleride(Base):

    def __init__(self, driver):
        self.driver = driver
        
    def Is(self):
        elems = self.driver.find_elements(By.CSS_SELECTOR, "button#submodel-filter[data-rd-tag=\"action-modelall\"]")
        return len(elems) > 0
    ...
    def run(self):
        ...
        elems = driver.find_elements(By.CSS_SELECTOR, "button.Link.SearchRefine-cat")
        elems = [x for x in elems if "GR86" in x.text and "build" not in x.text]
        
        if len(elems) != 1:
            # Find model dropdown
            elems = driver.find_elements(By.CSS_SELECTOR, "button#submodel-filter[data-rd-tag=\"action-modelall\"]")
        ...
        # Find all car links
        elems = driver.find_elements(By.CSS_SELECTOR, ".CarItem")
